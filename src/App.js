...
export default function App() {
  ...
  const filteredHistory = completedOrders
    ...
    .sort((a, b) => new Date(b.completedAt || b.targetDate) - new Date(a.completedAt || a.targetDate));

  const [formData, setFormData] = useState({
    id: '', cell: 'Cell 1', day: 'Monday', woNumber: '', modelName: '', qtyPlanned: '', qtyIn: '', qtyOut: '', targetDate: '', deadline: '', status: 'queued', completedAt: null, archived: false
  });

  const isPlanDisabled = userRole === 'VMI';
  const isActualDisabled = userRole === 'Prepare';
  const inputClass = (disabled) => `w-full border border-slate-300 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-[#e04616] focus:border-[#e04616] focus:outline-none ${disabled ? 'bg-slate-100 text-slate-500 cursor-not-allowed opacity-70' : ''}`;
  ...
  const handleOpenModal = (cell, day, existingWo = null) => {
    ...
    if (existingWo) {
      ...
      if (newForm.status === 'completed') newForm.completedAt = new Date().toISOString();
      setFormData(newForm);
    }
    ...
    reader.onload = (event) => {
      try {
        ...
        for (let i = 3; i < lines.length; i++) {
          ...
          let parsedTargetDate = parseCustomDate(planShip) || new Date().toISOString().split('T')[0];
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'running': return <PlayCircle className="w-4 h-4 mr-1" />;
  ...
  const getCellQueueInfo = (cellName) => {
    ...
    const running = cellWos.find(wo => wo.status === 'running') || cellWos.find(wo => wo.status === 'delayed');
    const queued = cellWos.filter(wo => wo.status === 'queued').sort((a, b) => DAYS.indexOf(a.day) - DAYS.indexOf(b.day))[0];
  ...
  const renderWorkOrderCard = (wo, isTray = false) => {
    ...
    return (
      <div 
        ...
        className={`border rounded-lg shadow-sm ${userRole !== 'VMI' ? 'cursor-grab active:cursor-grabbing' : 'cursor-pointer'} hover:shadow-md transition-shadow text-xs overflow-hidden flex flex-col bg-white ${isSelected ? 'border-[#e04616] ring-1 ring-[#e04616]' : 'border-slate-200'} ${isTray ? 'min-w-[220px] shrink-0' : ''}`}
      >
        {/* PLAN ROW */}
        <div className="bg-slate-100 p-2 border-b border-slate-200 relative">
            ...
            <div className="absolute top-2 right-2 z-10">
              <input 
                ...
                onChange={(e) => {
                  e.stopPropagation();
                  setSelectedWos(prev => 
                    prev.includes(wo.id) ? prev.filter(id => id !== wo.id) : [...prev, wo.id]
                  );
                }}
              />
            </div>
          ...
          <div className={`flex justify-between items-start mb-1 gap-2 ${isTray ? 'pr-5' : ''}`}>
            <span className="font-bold text-slate-800 truncate" title={wo.modelName}>{wo.modelName}</span>
            {!isTray && <span className="text-slate-500 font-medium text-[10px] uppercase bg-slate-200 px-1.5 py-0.5 rounded shrink-0">Plan</span>}
          </div>
          <div className="text-[10px] text-slate-500 font-mono mb-1">{wo.woNumber}</div>
          <div className="flex justify-between items-center text-slate-600 mt-1">
            <span className="text-xs">Qty: <span className="font-semibold">{wo.qtyPlanned}</span></span>
            <span className={`text-xs font-semibold flex items-center ${wo.status === 'delayed' || isTargetPassed ? 'text-red-600' : 'text-slate-700'}`}>
              Target: {formatDateShort(wo.targetDate)}
              {isTargetPassed && <AlertTriangle className="w-3 h-3 text-red-500 ml-1" title="Target date has passed" />}
            </span>
          </div>
          <div className="flex justify-end mt-0.5">
            <span className={`text-[9px] flex items-center gap-1 ${wo.targetDate > wo.deadline ? 'text-red-500 font-bold' : 'text-slate-400'}`}>
              {wo.targetDate > wo.deadline && <AlertTriangle className="w-3 h-3" />}
        ...
        <div className="p-2 bg-white relative">
           <div className="flex justify-between items-center mb-2">
              ...
              <span className={`px-2 py-0.5 rounded-full flex items-center text-[10px] font-bold uppercase border ${getStatusColor(wo.status)}`}>
           ...
           <div className="grid grid-cols-2 gap-2 text-center">
              ...
              <div className={`p-1 rounded border ${wo.qtyOut >= wo.qtyPlanned && wo.qtyPlanned > 0 ? 'bg-green-50 border-green-100' : 'bg-slate-50 border-slate-100'}`}>
                <div className="text-slate-400 text-[10px] uppercase">Output</div>
                <div className={`font-bold ${wo.qtyOut >= wo.qtyPlanned && wo.qtyPlanned > 0 ? 'text-green-700' : 'text-slate-700'}`}>{wo.qtyOut}</div>
              </div>
           </div>
        </div>
      </div>
    ...
    >
      ...
      <header className="bg-white border-b-4 border-[#e04616] px-4 py-3 sm:px-6 sm:py-4 shadow-sm sticky top-0 z-20 flex flex-wrap lg:flex-nowrap justify-between items-center gap-4 overflow-visible w-full">
        ...
        <div className="flex items-center gap-4 shrink-0">
          <svg viewBox="0 0 280 60" className="h-10 sm:h-12 w-auto shrink-0" xmlns="http://www.w3.org/2000/svg">
            <polygon points="0,60 15,0 35,0 20,60" fill="#e04616" />
            <text x="45" y="40" fontFamily="Georgia, serif" fontStyle="italic" fontWeight="bold" fontSize="42" fill="#e04616">Focuz</text>
            <text x="50" y="55" fontFamily="Arial, sans-serif" fontWeight="bold" fontSize="11" fill="#475569" letterSpacing="1">MANUFACTURING SERVICES</text>
          </svg>
          <div className="border-l-2 border-gray-200 pl-4 shrink-0">
            <h1 className="text-lg sm:text-xl font-bold uppercase tracking-wider mt-0.9 text-slate-700 whitespace-nowrap">SMT Schedule dashboard</h1>
            <p className="text-[#e04616] text-[10px] sm:text-xs font-bold uppercase tracking-wider mt-0.5 whitespace-nowrap">Production plan </p>
          </div>
          
          <div className="hidden md:flex ml-2 items-center gap-1 bg-slate-100 p-1 rounded-lg border border-slate-200 shrink-0">
            <button 
              onClick={() => setActiveView('dashboard')}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs sm:text-sm font-bold transition-all ${activeView === 'dashboard' ? 'bg-white shadow-sm text-[#e04616]' : 'text-slate-500 hover:text-slate-700'}`}
            >
              <LayoutDashboard className="w-4 h-4" /> Dashboard
            </button>
            <button 
              onClick={() => setActiveView('history')}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs sm:text-sm font-bold transition-all ${activeView === 'history' ? 'bg-white shadow-sm text-[#e04616]' : 'text-slate-500 hover:text-slate-700'}`}
            >
        ...
        <div className="md:hidden w-full flex items-center gap-1 bg-slate-100 p-1 rounded-lg border border-slate-200 mt-2">
            <button 
              onClick={() => setActiveView('dashboard')}
              className={`flex-1 flex justify-center items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-bold transition-all ${activeView === 'dashboard' ? 'bg-white shadow-sm text-[#e04616]' : 'text-slate-500 hover:text-slate-700'}`}
            >
              <LayoutDashboard className="w-4 h-4" /> Dashboard
            </button>
            <button 
              onClick={() => setActiveView('history')}
              className={`flex-1 flex justify-center items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-bold transition-all ${activeView === 'history' ? 'bg-white shadow-sm text-[#e04616]' : 'text-slate-500 hover:text-slate-700'}`}
            >
              <History className="w-4 h-4" /> History Log
            </button>
        </div>
          ...
          >
            <div className="min-w-[1200px] bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              <div className="grid grid-cols-6 border-b border-slate-200 bg-slate-50 text-slate-600 font-semibold text-sm uppercase tracking-wider">
                  <div className="p-4 border-r border-slate-200 text-center">Cell / Queue</div>
                  {DAYS.map(day => (
                    <div key={day} className="p-4 border-r border-slate-200 text-center last:border-r-0">
                      {day}
                    </div>
                  ))}
                </div>
              ...
              {CELLS.map(cell => {
                ...
                return (
                  <div key={cell} className="grid grid-cols-6 border-b border-slate-200 last:border-b-0 hover:bg-slate-50 transition-colors">
                    
                    <div className="p-4 border-r border-slate-200 bg-white flex flex-col justify-center">
                      <h2 className="text-lg font-bold text-slate-800 mb-3">{cell}</h2>
                      
                      <div className="space-y-2 text-xs">
                        <div className="bg-blue-50 border border-blue-100 rounded p-2">
                          <div className="text-blue-800 font-semibold mb-1 flex items-center"><PlayCircle className="w-3 h-3 mr-1"/> Current</div>
                          {queueInfo.running ? (
                            <div className="font-bold text-slate-800 truncate" title={queueInfo.running.modelName}>
                        ...
                        <div className="bg-gray-50 border border-gray-200 rounded p-2">
                          <div className="text-slate-600 font-semibold mb-1 flex items-center"><Clock className="w-3 h-3 mr-1"/> Next Queue</div>
                          {queueInfo.queued ? (
                            <div className="font-bold text-slate-800 truncate" title={queueInfo.queued.modelName}>
                    ...
                    {DAYS.map(day => {
                      const dayWos = dashboardOrders.filter(wo => wo.cell === cell && wo.day === day);
          ...
          <div className="bg-slate-100 border-t border-slate-300 p-4 shrink-0 shadow-[0_-4px_10px_rgba(0,0,0,0.05)] z-10 relative">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3 gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                ...
                {userRole !== 'VMI' && (
                   <button 
                     onClick={() => handleOpenModal('Unscheduled', 'Unscheduled')}
                ...
                {selectedWos.length > 0 && userRole !== 'VMI' && (
                   ...
                   >
                     <Trash2 className="w-3.5 h-3.5" /> Delete Selected ({selectedWos.length})
                   </button>
                )}
              </div>
            </div>
            ...
            <div 
              ...
              onDragOver={handleDragOver}
              onDrop={(e) => {
                if (userRole !== 'VMI') handleDrop(e, 'Unscheduled', 'Unscheduled');
              }}
            >
            ...
            <div className="p-6 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-50">
              <div>
                <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                  <CheckCircle className="w-5 h-5 text-green-500" /> Production History Log
                </h2>
                <p className="text-sm text-slate-500 mt-1">Record of completed jobs to help identify previous routing cells.</p>
              </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-100 text-slate-600 text-xs uppercase tracking-wider border-b border-slate-200">
                    ...
                    <th className="p-4 font-semibold text-[#e04616]">Production Cell</th>
                    ...
                    filteredHistory.map(wo => (
                      <tr key={wo.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-4 font-bold text-slate-800">
                          {wo.cell !== 'Unscheduled' ? wo.cell : <span className="text-slate-400 font-normal italic">N/A</span>}
                        </td>
                        <td className="p-4 font-bold text-slate-800">{wo.modelName}</td>
                        <td className="p-4 text-slate-500 font-mono text-xs">{wo.woNumber}</td>
                        <td className="p-4 text-right text-slate-600">{wo.qtyPlanned}</td>
                        <td className="p-4 text-right font-bold text-green-700">{wo.qtyOut}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td colSpan="7" className="p-12 text-center text-slate-500">
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <p className="text-lg font-medium text-slate-600">No completed records found</p>
                          <p className="text-sm">
                            {historySearch ? 'Try adjusting your search terms.' : 'Jobs will appear here once Actual Output meets Planned Quantity.'}
                          </p>
                        </div>
                      </td>
                    </tr>
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-xl overflow-hidden animate-in fade-in zoom-in duration-200 relative my-8">
              ...
              <div className="bg-slate-800 px-6 py-4 flex justify-between items-center text-white sticky top-0 z-10">
                <h3 className="text-lg font-bold">
                  {editingWo 
                    ? (userRole === 'VMI' ? 'Inspect Actuals' : 'Update Production Status') 
                    : 'Schedule Work Order'}
                </h3>
              ...
              <form onSubmit={handleSave} className="p-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Cell</label>
                    <select 
                      disabled={isPlanDisabled}
                      className={inputClass(isPlanDisabled)}
                      value={formData.cell} onChange={(e) => setFormData({...formData, cell: e.target.value})}
                    >
                ...
                <div className={`bg-slate-50 p-4 rounded-lg border border-slate-200 mb-6 space-y-4 ${isPlanDisabled ? 'opacity-80' : ''}`}>
                  <h4 className="text-sm font-bold text-slate-800 border-b border-slate-200 pb-2 flex justify-between">
                    <span>Production Plan {isPlanDisabled && '(Read Only)'}</span>
                    {isPlanDisabled && <span className="text-xs text-slate-500 font-normal">Prepare Team Only</span>}
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Work Order (WO)</label>
                      <input required={!isPlanDisabled} disabled={isPlanDisabled} type="text" className={inputClass(isPlanDisabled)} placeholder="e.g. WO-10294"
                        value={formData.woNumber} onChange={(e) => setFormData({...formData, woNumber: e.target.value})} autoFocus={!editingWo && !isPlanDisabled} />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Model Name</label>
                      <input required={!isPlanDisabled} disabled={isPlanDisabled} type="text" className={inputClass(isPlanDisabled)} placeholder="e.g. PCB-MAIN"
                    ...
                    <div>
                      ...
                      <input required={!isPlanDisabled} disabled={isPlanDisabled} type="date" className={inputClass(isPlanDisabled)}
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase mb-1 flex justify-between items-center">
                        ...
                        {formData.targetDate > formData.deadline && (
                          <span className="text-red-500 flex items-center normal-case"><AlertTriangle className="w-3 h-3 mr-1"/> Exceeded</span>
                        )}
                      </label>
                      <input required={!isPlanDisabled} disabled={isPlanDisabled} type="date" className={`${inputClass(isPlanDisabled)} ${formData.targetDate > formData.deadline ? 'border-red-500 ring-red-200 focus:ring-red-500' : ''}`}
                      ...
                      <input 
                        ...
                        onChange={(e) => {
                          ...
                          setFormData(prev => {
                            ...
                            const newStatus = calculateStatus({...nextState, qtyPlanned: val || 0}, prev.day);
                            if (newStatus === 'completed' && prev.status !== 'completed') {
                              nextState.completedAt = new Date().toISOString();
                            }
                            nextState.status = newStatus;
                ...
                <div className={`bg-[#e04616]/5 p-4 rounded-lg border border-[#e04616]/20 mb-6 space-y-4 ${isActualDisabled ? 'opacity-80' : ''}`}>
                   <div className="flex justify-between items-center border-b border-[#e04616]/20 pb-2">
                      <h4 className="text-sm font-bold text-[#e04616] flex items-center gap-2">
                        Actual Production {isActualDisabled && <span className="text-xs text-[#e04616]/70 font-normal">(Read Only)</span>}
                      </h4>
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-[#e04616]/70 font-normal hidden sm:block">
                          {isActualDisabled ? 'VMI Team Only' : '(Auto-calculated)'}
                        </span>
                        <select 
                          disabled={isActualDisabled}
                          title="Status is calculated automatically based on Day and Quantity"
                          className={`text-xs font-bold uppercase rounded-full px-3 py-1 border outline-none ${isActualDisabled ? 'cursor-not-allowed opacity-70' : 'cursor-pointer'} ${getStatusColor(formData.status)}`}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-[#e04616] uppercase mb-1">Quantity Input</label>
                      <input 
                        disabled={isActualDisabled} 
                        type="number" min="0" 
                    ...
                    <div>
                      <label className="block text-xs font-bold text-[#e04616] uppercase mb-1">Quantity Output</label>
                      <input 
                        disabled={isActualDisabled} 
                        type="number" min="0" 
                        className={inputClass(isActualDisabled)}
                        ...
                        onChange={(e) => {
                          ...
                          setFormData(prev => {
                            ...
                            const newStatus = calculateStatus({...nextState, qtyOut: val || 0}, prev.day);
                            if (newStatus === 'completed' && prev.status !== 'completed') {
                              nextState.completedAt = new Date().toISOString();
                            }
                            nextState.status = newStatus;
                ...
                <div className="flex flex-col-reverse sm:flex-row justify-between items-center gap-4 pt-4 border-t border-slate-200">
                  {editingWo && !isPlanDisabled ? (
                    <button type="button" onClick={handleDelete} className="w-full sm:w-auto text-red-600 text-sm font-semibold hover:text-red-800 hover:bg-red-50 rounded-md px-4 py-2 transition-colors">
                      Delete Order
                    </button>
                  ) : <div className="hidden sm:block"></div>}
                  <div className="flex w-full sm:w-auto gap-3">
                    <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 sm:flex-none px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100 rounded-md transition-colors text-center border border-slate-300 sm:border-transparent">
                      Cancel
                    </button>
                    <button type="submit" className="flex-1 sm:flex-none px-4 py-2 text-sm font-semibold bg-[#e04616] text-white rounded-md hover:bg-[#c43a10] transition-colors shadow-sm text-center">
